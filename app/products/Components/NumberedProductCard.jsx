...
const NumberedProductCard = React.memo(function NumberedProductCard({
  ...
  const [upvoteCount, setUpvoteCount] = useState(product?.upvoteCount ?? product?.upvotes?.count ?? 0);
  const [hasUpvoted, setHasUpvoted] = useState(product?.upvoted ?? product?.upvotes?.userHasUpvoted ?? product?.userInteractions?.hasUpvoted ?? false);
  const [bookmarkCount, setBookmarkCount] = useState(product?.bookmarkCount ?? product?.bookmarks?.count ?? 0);
  const [hasBookmarked, setHasBookmarked] = useState(product?.bookmarked ?? product?.bookmarks?.userHasBookmarked ?? product?.userInteractions?.hasBookmarked ?? false);

  // Update state when product props change
  useEffect(() => {
    setUpvoteCount(product?.upvoteCount ?? product?.upvotes?.count ?? 0);
    setHasUpvoted(product?.upvoted ?? product?.upvotes?.userHasUpvoted ?? product?.userInteractions?.hasUpvoted ?? false);
    setBookmarkCount(product?.bookmarkCount ?? product?.bookmarks?.count ?? 0);
    setHasBookmarked(product?.bookmarked ?? product?.bookmarks?.userHasBookmarked ?? product?.userInteractions?.hasBookmarked ?? false);
  }, [product]);
