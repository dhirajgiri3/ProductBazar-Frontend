...
const DashboardPreview = () => {
  ...
  // Debounce mouse move updates for performance
  ...
  const handleMouseLeave = () => {
    setIsHovering(false);
    // Reset mouse values smoothly when mouse leaves
    if (containerRef.current) {
  ...
  // Effect to initialize mouse position to center after mount
  useEffect(() => {
    if (containerRef.current) {
      mouseX.set(containerRef.current.offsetWidth / 2);
      mouseY.set(containerRef.current.offsetHeight / 2);
    }
  }, [mouseX, mouseY]); // Only run once after mount

  // Smooth transform values using spring physics
  const springConfig = { stiffness: 150, damping: 20, mass: 1 };

  // Dynamically update range based on container size when available
  ...
  const dashboardScreens = [imageUrl1, imageUrl2, imageUrl3, imageUrl4];
  ...
  // --- Animation Variants ---
  // These are defined inside the component and passed down
