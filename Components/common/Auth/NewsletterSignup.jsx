...
const NewsletterSignup = ({ className = '' }) => {
  ...
  return (
    <motion.div 
      className={`bg-gradient-to-br from-indigo-50 to-violet-50 rounded-xl p-6 shadow-sm border border-indigo-100 ${className}`}
    ...
    >
      ...
      >
        <input
          ...
          className={`w-full px-4 py-3 pr-12 rounded-lg text-sm focus:outline-none focus:ring-2 
            ${status === 'error' 
              ? 'focus:ring-red-500 border-red-300 bg-red-50' 
              : 'focus:ring-indigo-500 border-gray-200 bg-white'} 
            transition-all`}
        />
        ...
        <motion.button
          type="submit"
          disabled={status === 'loading' || status === 'success'}
          className={`absolute right-1.5 top-1.5 p-1.5 rounded-md
            ${status === 'loading' 
      ...
      <motion.p 
        className="text-gray-500 text-xs mt-4 text-center italic"
        variants={itemVariants}
      >
