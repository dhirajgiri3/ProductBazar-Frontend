    ...
    if (token) {
      ...
      if (config.url?.includes('/recommendations')) {
        ...
        console.log('API request with token:', {
          ...
          authorizationHeader: config.headers['Authorization'] ? 'present' : 'missing'
        });
      ...
      if (config.url?.includes('/recommendations')) {
        ...
        console.log('API request without token:', {
          url: config.url,
          hasToken: false,
          sessionStorageToken: sessionStorage.getItem('accessToken') ? 'exists' : 'missing',
          authorizationHeader: config.headers['Authorization'] ? 'present' : 'missing'
        });
        console.log('=== END API REQUEST DEBUG ===');
      }
...
export const makePriorityRequest = async (method, url, options = {}) => {
  ...
  // User authentication endpoints should use caching to prevent duplicate calls
      const shouldUseCache = useCache || url.includes('/auth/profile');
