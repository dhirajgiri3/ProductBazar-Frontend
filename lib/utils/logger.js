...
const logger = {
  ...
  error: (...args) => {
    ...
    if (process.env.NODE_ENV === 'production') {
      ...
      try {
        // Log to localStorage for debugging (limited storage)
        const errorLog = {
        ...
        // Optionally send to server for centralized logging
        if (typeof window !== 'undefined' && process.env.NEXT_PUBLIC_API_URL) {
          fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/admin/logs/error`, {
            method: 'POST',
            headers: {
